...
export const AddtoCart = async ({ userId, name, price, quantity, product_id, image }) => {
  try {
    const carts = await axios.post("/api/cart", { userId, name, price, quantity, product_id, image });
...
export async function createOrder(data) {
  try {
    ...
    const response2 = await axios.post('/api/orderitem', { order_id: response.data.data.order_id, cartItems: data.cartItems }, {
      headers: {
        'Content-Type': 'application/json'
      }
    })
